	...
	if (nodes.length > 0 && nodes[0].id != 0) {
		common.ajax({
			...
			success : function(data) {
				...
				// 将菜单树上,用户组对应的菜单节点勾选上
				for (var i = 0; i < data.menuListDto.length; i++) {
					// 因为菜单树是一颗混合树,需要用组合ID(带前缀的ID)来选中对应的节点
					menuTree.checkNode(menuTree.getNodeByParam("comboId", common.menuPrefix.PREFIX_MENU + data.menuListDto[i].id), true);
				}
				
				// 将菜单树上,用户组对应的动作节点勾选上
				for (var i = 0; i < data.actionListDto.length; i++) {
					// 因为菜单树是一颗混合树,需要用组合ID(带前缀的ID)来选中对应的节点
					menuTree.checkNode(menuTree.getNodeByParam("comboId", common.menuPrefix.PREFIX_ACTION + data.actionListDto[i].id), true);
				}
			}
		});
	}
